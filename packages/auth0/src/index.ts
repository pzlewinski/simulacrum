...
type Auth0Handlers = ReturnType<typeof createAuth0Handlers> &
  ReturnType<typeof createOpenIdHandlers>;

const createAuth0Service = (
  handlers: Auth0Handlers,
  { port, debug }: { port: number; debug: boolean }
): LegacyServiceCreator => {
...
export const auth0: Simulator<Options> = (slice, options) => {
  ...
  return {
    services: {
      auth0: createAuth0Service(
        { ...auth0Handlers, ...openIdHandlers },
        serviceOptions
      ),
    },
    scenarios: {
